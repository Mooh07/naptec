...
class FacetRemove extends HTMLElement {
  constructor() {
    ...
    // this.checkFilters_hideRemoveAllButton();
  }
  ...
  checkFilters_hideRemoveAllButton() {
    let currentUrl = window.location.href;
    let url = new URL(currentUrl);
    let queryParams = new URLSearchParams(url.search);
    let isFilterd = queryParams.toString().length > 0 && queryParams.includes('filter.v');
    if (!isFilterd) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const selectElementMobile = document.getElementById('per-page-mobile');
